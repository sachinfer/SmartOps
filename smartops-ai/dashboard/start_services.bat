...
if not exist "event_api.py" (
    ...
    exit /b 1
        ...
        if not exist "page_router.py" (
            echo ❌ Please run this script from the dashboard directory
            echo Expected files: event_api.py, page_router.py
            pause
            exit /b 1
        )
...
echo 🎨 Starting Streamlit frontend service...
        start "Streamlit Frontend" cmd /k "streamlit run page_router.py --server.port=8501 --server.address=0.0.0.0"
